    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (request.getParameter("submit") != null) {
                ...
                String bookIdRadio = request.getParameter("bookIdRadio");

                if (bookIdRadio.equals("1")) {
                    bookId += "_001";
                } else {
                    String[] bookIdNumber = bookId.split("_");
                    bookId = bookIdNumber[0].toUpperCase() + "_" + new DecimalFormat("000").format(Double.parseDouble(bookIdNumber[1]));
                }

                bdb.insert(new Book(bookId, categoryId, publicationId, title, publicationDate, price, quantity, notes, "image/" + image, authorId));
